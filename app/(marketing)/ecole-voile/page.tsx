...
const SCHOOL_LEVELS = [
  {
    ...
    gallery: [
      "https://images.unsplash.com/photo-1596423736772-799a4e3df530?q=80&w=1600&auto=format&fit=crop",
      "https://images.unsplash.com/photo-1516686120803-03099958197c?q=80&w=1600&auto=format&fit=crop",
      "https://images.unsplash.com/photo-1540946485063-a40da27545f8?q=80&w=1600&auto=format&fit=crop"
    ],
    color: "text-orange-500",
    bgColor: "bg-orange-500",
  ...
  },
  {
    ...
    gallery: [
      "https://images.unsplash.com/photo-1562677765-a8775df50b4e?q=80&w=1600&auto=format&fit=crop",
      "https://images.unsplash.com/photo-1534008897995-27a23e859048?q=80&w=1600&auto=format&fit=crop",
      "https://images.unsplash.com/photo-1471922694854-ff1b63b20054?q=80&w=1600&auto=format&fit=crop"
    ],
    color: "text-turquoise",
    bgColor: "bg-turquoise",
  ...
  },
  {
    ...
    gallery: [
      ...
      "https://images.unsplash.com/photo-1500930287596-c1ecaa373bb2?q=80&w=1600&auto=format&fit=crop",
      "https://images.unsplash.com/photo-1563462058316-29a399f665e7?q=80&w=1600&auto=format&fit=crop"
    ],
    color: "text-blue-600",
    bgColor: "bg-blue-600",
  ...
  },
  {
    ...
    gallery: [
      ...
      "https://images.unsplash.com/photo-1473663175406-03c6237227d8?q=80&w=1600&auto=format&fit=crop",
      "https://images.unsplash.com/photo-1628173429871-3f0e01764491?q=80&w=1600&auto=format&fit=crop"
    ],
    color: "text-purple-500",
    bgColor: "bg-purple-500",
...
const getActivityIcon = (type: string) => {
    switch(type) {
        case 'pool': return <LifeBuoy size={16} />;
        case 'boat': return <Ship size={16} />;
        case 'wind': return <Wind size={16} />;
        case 'cata': return <Anchor size={16} />;
        case 'sup': return <Waves size={16} />;
        case 'kite': return <Wind size={16} />;
        case 'map': return <Map size={16} />;
        case 'trophy': return <Trophy size={16} />;
        case 'fish': return <Waves size={16} />;
...
export const EcoleVoilePage: React.FC = () => {
  ...
  const visibleLevels = activeTab === 'all' 
    ? SCHOOL_LEVELS 
    : SCHOOL_LEVELS.filter(l => l.id === activeTab);

  const currentWeek = PLANNING_WEEKS[currentWeekIndex];

  const nextWeek = () => {
    setCurrentWeekIndex(prev => (prev + 1) % PLANNING_WEEKS.length);
  };

  const prevWeek = () => {
    setCurrentWeekIndex(prev => (prev - 1 + PLANNING_WEEKS.length) % PLANNING_WEEKS.length);
  };
              ...
              {SCHOOL_LEVELS.map((level) => (
                ...
                >
                   ...
                   <div className="text-left">
                      ...
                      <span className={`block text-sm font-black italic tracking-tighter leading-none ${activeTab === level.id ? 'text-abysse' : 'text-slate-500'}`}>
                          {level.title}
                      </span>
                   </div>
                </button>
               ...
               <div className={`flex flex-col xl:flex-row gap-16 items-start`}>
                 ...
                 <div className={`w-full xl:w-[45%] shrink-0 ${index % 2 === 1 ? 'xl:order-2' : ''}`}>
                    <div className="sticky top-40">
                        <div className="h-[400px] md:h-[600px] rounded-[2.5rem] overflow-hidden shadow-2xl relative">
      ...
      <section className="py-24 px-6 bg-slate-100">
          <div className="max-w-[1600px] mx-auto">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                {/* BLOC 3 : A PRÉVOIR */}
                <div className="bg-abysse text-white p-8 rounded-[2.5rem] shadow-lg relative overflow-hidden">
         ...
         <div className="max-w-[1600px] mx-auto relative z-10">
            ...
            <div className="flex items-center justify-center gap-6 mb-16">
               ...
               <div className="bg-white/5 border border-white/10 rounded-2xl px-12 py-6 text-center min-w-[340px] backdrop-blur-md shadow-2xl">
                  ...
                  <span className="block text-2xl font-black italic">{currentWeek.label}</span>
                  <div className="flex items-center justify-center gap-6 mt-3 pt-3 border-t border-white/10">
                     <span className="text-xs font-bold text-turquoise uppercase tracking-widest">Coeff {currentWeek.coefficient}</span>
                     <span className="w-1 h-1 bg-white/20 rounded-full"></span>
                     <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Marée {currentWeek.tide}</span>
                  </div>
               </div>
                      ...
                      return (
                        <div key={row.id} className="grid grid-cols-6 gap-6 mb-6 group/row">
                            ...
                            {currentWeek.days.map((day, dayIdx) => {
                                ...
                                if (!data) return <div key={dayIdx} className="bg-white/5 rounded-2xl"></div>;

                                return (
                                    <div key={dayIdx} className={`relative bg-white/5 backdrop-blur-md rounded-2xl p-5 flex flex-col justify-center border border-white/5 hover:bg-white/10 transition-colors group/cell overflow-hidden ${data.isRaid ? 'bg-yellow-500/10 border-yellow-500/30' : ''}`}>
                                        
                                        {/* Colored Left Border Accent */}
                                        <div className={`absolute left-0 top-4 bottom-4 w-1 rounded-r-full ${data.isRaid ? 'bg-yellow-500' : row.bgColor}`}></div>
                                        ...
                                        <div className="pl-4 relative z-10">
                                            ...
                                            <div className="flex items-baseline gap-2 mb-2">
                                                <span className="text-3xl font-black text-white tracking-tighter leading-none">{data.start}</span>
                                                <span className="text-lg font-medium text-white/30">-</span>
                                                <span className="text-lg font-bold text-white/60">{data.end}</span>
                                            </div>
                                            
                                            {data.activity && (
                                                <div className={`text-[10px] font-bold uppercase tracking-widest truncate ${data.isRaid ? 'text-yellow-400' : row.color}`}>
                                                    {data.activity}
                                                </div>
                                            )}
                                        ...
                                        <div className={`absolute -right-4 -bottom-4 opacity-[0.03] text-white scale-150 group-hover/cell:scale-125 transition-transform duration-500 rotate-12`}>
                                            {data.icon ? getActivityIcon(data.icon) : row.icon}
                                        </div>
                                    </div>
                                );
                            })}
