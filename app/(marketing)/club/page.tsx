        ...
        <section id="identity" className="py-24 px-6 max-w-[1400px] mx-auto">
            ...
            <div className="mb-24">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {/* Transmission */}
                    <div className="bg-slate-50 p-10 rounded-[2.5rem] border border-slate-100 relative overflow-hidden group hover:bg-white hover:shadow-xl transition-all duration-500">
            ...
            <div className="relative rounded-[3rem] overflow-hidden h-[750px] shadow-2xl group border border-slate-900 bg-abysse">
                ...
                <div className="absolute inset-0 z-20 pointer-events-none flex">
                    ...
                    <div className="flex-1 relative pointer-events-none">
                        ...
                        <div className="absolute inset-y-0 left-0 w-24 hover:bg-gradient-to-r hover:from-black/20 hover:to-transparent transition-all cursor-pointer pointer-events-auto flex items-center justify-start group" onClick={prevImage}>
                             <div className="p-4 opacity-0 group-hover:opacity-100 transition-opacity"><ChevronLeft className="text-white" size={48} /></div>
                        </div>
                        <div className="absolute inset-y-0 right-0 w-24 hover:bg-gradient-to-l hover:from-black/20 hover:to-transparent transition-all cursor-pointer pointer-events-auto flex items-center justify-end group" onClick={nextImage}>
        ...
        <section id="life" className="py-24 px-6 bg-slate-50 border-y border-slate-200">
            <div className="max-w-[1400px] mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {/* Volunteer Call */}
                    <div className="bg-abysse text-white p-10 rounded-[2.5rem] flex flex-col justify-center text-center relative overflow-hidden">
            ...
            <div className="max-w-[1400px] mx-auto">
                ...
                <div className="space-y-12">
                    ...
                    <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden relative">
                        <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-abysse via-turquoise to-abysse"></div>
                    ...
                    <div>
                        <h3 className="text-3xl font-black text-abysse uppercase italic mb-8 flex items-center gap-4 pl-4">
                            <span className="size-14 bg-orange-500 rounded-2xl flex items-center justify-center text-white shadow-lg rotate-[-5deg]"><Zap size={28}/></span>
                            2. Séances Découverte
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {[
                                { name: "Char à Voile", detail: "2h | dès 8 ans", price: "35 €", icon: "wind" },
                                { name: "Trimaran Été", detail: "2h | dès 5 ans (+adulte)", price: "35 €", icon: "sailing" },
                                { name: "Catamaran Été", detail: "2h | dès 8 ans", price: "35 €", icon: "anchor" },
                                { name: "Marche Aquatique", detail: "1h | dès 14 ans", price: "10 €", icon: "waves" },
                                { name: "Speed Sail", detail: "1h30 | dès 14 ans", price: "50 €", icon: "speed" },
                                { name: "Kitesurf", detail: "Séance unité (max 3 pers)", price: "140 €", icon: "kitesurfing" },
                            ].map((card, i) => (
                                <div key={i} className="bg-white p-8 rounded-[2rem] border border-slate-100 hover:border-turquoise shadow-sm hover:shadow-xl transition-all duration-300 group relative overflow-hidden">
                                    <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 group-hover:scale-125 transition-all duration-500 text-abysse">
                                        <span className="material-symbols-outlined text-6xl">{card.icon === 'wind' ? 'air' : 'sailing'}</span>
                                    </div>
                                    <div className="relative z-10">
                                        <h4 className="font-black text-abysse text-xl mb-2 leading-tight min-h-[3rem] flex items-end">{card.name}</h4>
                                        <div className="w-10 h-1 bg-slate-100 group-hover:bg-turquoise transition-colors rounded-full mb-4"></div>
                                        <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-6">{card.detail}</p>
                                        <div className="flex justify-end items-end">
                                            <span className="text-4xl font-black text-abysse group-hover:text-turquoise transition-colors">{card.price}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
                            
                            {/* Wing Foil Special Card */}
                            <div className="md:col-span-2 bg-abysse p-8 rounded-[2rem] border border-slate-800 text-white relative overflow-hidden shadow-2xl group">
                                <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-turquoise/10 rounded-full blur-[80px] pointer-events-none group-hover:bg-turquoise/20 transition-colors"></div>
                                <div className="relative z-10 h-full flex flex-col justify-between">
                                    <div className="flex justify-between items-start mb-8">
                                        <div>
                                            <h4 className="font-black text-3xl mb-2 italic">Wing Foil</h4>
                                            <span className="inline-block px-3 py-1 bg-turquoise/20 text-turquoise border border-turquoise/30 rounded-lg text-[10px] font-black uppercase tracking-widest">
                                                Tendance 2025
                                            </span>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Dès 14 ans</p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-3 gap-4">
                                        <div className="bg-white/5 backdrop-blur-sm p-4 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors">
                                            <p className="text-[9px] uppercase font-bold text-slate-400 mb-1">Séance</p>
                                            <p className="font-black text-2xl">130 €</p>
                                        </div>
                                        <div className="bg-white/5 backdrop-blur-sm p-4 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors relative overflow-hidden">
                                            <div className="absolute top-0 right-0 w-8 h-8 bg-turquoise/50 blur-xl"></div>
                                            <p className="text-[9px] uppercase font-bold text-turquoise mb-1">3 Séances</p>
                                            <p className="font-black text-2xl text-white">330 €</p>
                                        </div>
                                        <div className="bg-white/5 backdrop-blur-sm p-4 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors relative overflow-hidden">
                                            <div className="absolute top-0 right-0 w-8 h-8 bg-purple-500/50 blur-xl"></div>
                                            <p className="text-[9px] uppercase font-bold text-purple-400 mb-1">5 Séances</p>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        {/* LOCATIONS */}
                        <div className="lg:col-span-2 bg-white border border-slate-100 rounded-[2.5rem] p-8 md:p-10 shadow-lg">
                                    ...
                                    <tbody className="text-sm font-bold text-slate-700">
                                        {[
                                            ...
                                            <tr key={i} className="hover:bg-slate-50 transition-colors rounded-lg">
                                                <td className="py-3 pl-4 font-black text-abysse">{row.name}</td>
                                                <td className="py-3 text-center text-slate-500">{row.h1}</td>
                                                <td className="py-3 text-center text-slate-500">{row.h15}</td>
                                                <td className="py-3 text-center text-slate-500">{row.h2}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        {/* COURS PARTICULIERS */}
                        <div className="bg-slate-900 text-white rounded-[2.5rem] p-10 flex flex-col justify-center relative overflow-hidden shadow-2xl group">
                            {/* Animated Background */}
                            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5"></div>
                            <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-turquoise/20 rounded-full blur-[80px] group-hover:bg-turquoise/30 transition-colors"></div>
                            
                            <div className="relative z-10">
                                <h3 className="text-2xl font-black uppercase italic mb-10 flex items-center gap-4">
                                    <span className="size-12 bg-white/10 rounded-2xl flex items-center justify-center text-turquoise border border-white/10"><GraduationCap size={24}/></span>
                                    4. Cours Particuliers
                                </h3>
                                <div className="space-y-8">
                                    <div className="pb-6 border-b border-white/10">
                                        <div className="flex justify-between items-end mb-2">
                                            <p className="font-bold text-lg">Prestation Moniteur</p>
                                            <p className="text-4xl font-black text-turquoise">40 €</p>
                                        </div>
                                        <p className="text-xs text-slate-400 font-bold uppercase tracking-wider flex items-center gap-2">
                                            <span className="size-1.5 bg-turquoise rounded-full"></span> Par Heure
                                        </p>
                                    </div>
                                    <div>
                                        <div className="flex justify-between items-end mb-2">
                                            <p className="font-bold text-lg">Frais Matériel</p>
                                            <p className="text-4xl font-black text-white">20 €</p>
                                        </div>
                                        <p className="text-xs text-slate-400 font-bold uppercase tracking-wider flex items-center gap-2">
                                            <span className="size-1.5 bg-white rounded-full"></span> Engin de sécurité inclus
                                        </p>
                                    </div>
                                </div>
                                <button className="w-full mt-12 bg-white text-abysse py-5 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-turquoise hover:text-white transition-all shadow-lg flex items-center justify-center gap-3">
                    ...
                    <div className="bg-slate-900 rounded-[2.5rem] p-10 md:p-16 relative overflow-hidden text-white shadow-2xl">
                        {/* Decoration */}
                        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-turquoise/10 rounded-full blur-[100px] pointer-events-none"></div>
                        ...
                        <div className="relative z-10">
                            <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-12 gap-6">
                                <div>
                                    <h3 className="text-3xl font-black uppercase italic flex items-center gap-4 mb-2">
                                        <span className="size-14 bg-turquoise rounded-2xl flex items-center justify-center text-white shadow-lg border border-white/10"><HeartHandshake size={28}/></span>
                                        5. Club à l'Année
                                    </h3>
                                    <p className="text-turquoise font-bold text-lg pl-2">Saison 2023/2024</p>
                                </div>
                                <div className="bg-white/10 backdrop-blur-md px-6 py-3 rounded-xl border border-white/10 max-w-md">
                                    <p className="text-xs font-medium text-slate-300 flex gap-2">
                                        <Info size={16} className="shrink-0 text-turquoise" />
                                        Les tarifs indiqués incluent l'adhésion au club et la licence annuelle (adulte ou enfant).
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {[
                                    { title: "Char à Voile", age: "11 ans et +", schedule: "Samedis (Sept à Juin)", price: "264 €", subPrice: "225 € Ados" },
                                    { title: "Multi-activité", age: "8-11 ans", schedule: "Mercredis PM (Sept-Déc & Mars-Juin)", price: "164.50 €", subPrice: null },
                                    { title: "Planche à voile", age: "14 ans et +", schedule: "Pratique régulière", price: "253.50 €", subPrice: null },
                                    { title: "Catamaran 11-15", age: "11-15 ans", schedule: "Samedis (Sept à Juin)", price: "219.50 €", subPrice: null },
                                    { title: "Catamaran Ados", age: "13-17 ans", schedule: "Samedis (Sept-Déc & Mars-Juin)", price: "222 €", subPrice: null },
                                    { title: "Catamaran Adultes", age: "18 ans +", schedule: "Samedis (Sept-Déc & Mars-Juin)", price: "273 €", subPrice: null },
                                    { title: "Marche Aquatique", age: "14 ans et +", schedule: "Mercredis & Samedis (Sept à Juin)", price: "122 €", subPrice: null },
                                ].map((item, i) => (
                                    <div key={i} className="bg-white/5 border border-white/10 p-6 rounded-2xl hover:bg-white/10 transition-colors group">
                                        <div className="flex justify-between items-start mb-4">
                                            <h4 className="font-black text-xl italic">{item.title}</h4>
                                            <span className="bg-turquoise/20 text-turquoise text-[10px] font-black px-2 py-1 rounded uppercase tracking-wider">{item.age}</span>
                                        </div>
                                        <div className="flex items-start gap-3 mb-6 min-h-[40px]">
                                            <Calendar size={16} className="text-slate-400 mt-0.5 shrink-0" />
                                            <p className="text-xs text-slate-300 font-medium leading-snug">{item.schedule}</p>
                                        </div>
                                        <div className="border-t border-white/10 pt-4 flex items-end justify-between">
                                            <div>
                                                <p className="text-3xl font-black text-white tracking-tighter">{item.price}</p>
                                                {item.subPrice && <p className="text-xs text-slate-400 font-bold">{item.subPrice}</p>}
                                            </div>
                                        </div>
                                    </div>
                                ))}
