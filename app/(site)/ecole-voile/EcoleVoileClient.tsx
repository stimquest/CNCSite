...
const EcoleVoileClient: React.FC<EcoleVoileClientProps> = ({ initialSchoolPageData, initialPlannings }) => {
  const planningsData = initialPlannings;
  ...
  return (
    <div className="w-full font-sans bg-white">
      ...
      >
        {schoolPageData?.heroBadges?.map((badge: any, idx: number) => {
      ...
      <SecondaryNav sections={(schoolPageData?.stages?.length ? schoolPageData.stages : DEFAULT_STAGES).map((s: any) => ({
        id: s.id,
      ...
      <section className="relative bg-white">
        ...
        {(schoolPageData?.stages?.length ? schoolPageData.stages : DEFAULT_STAGES).map((item: any, index: number) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-10 rounded-[3rem] hover:bg-white/10 transition-all group">
              ...
              <ul className="space-y-4">
                {(schoolPageData?.equipmentProvided || ["Combinaisons intégrales", "Gilets de sauvetage", "Harnais de trapèze", "Coupe-vent"]).map((item: any, i: number) => (
