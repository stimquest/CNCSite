...
export const PlanningWidget: React.FC = () => {
    ...
    const renderVoileTable = (week: any) => (
        <div className="overflow-x-auto pb-4">
            <table className="w-full min-w-[800px] border-separate border-spacing-1">
                <thead>
                    <tr>
                ...
                <tbody>
                    {GROUP_CONFIG.map((group) => (
                        <tr key={group.id}>
                            <td className={`p-3 rounded-xl border ${group.border} ${group.bg} shadow-sm`}>
                            ...
                            {week.days?.map((day: any, dayIdx: number) => {
                                ...
                                if (isRaid) return (
                                    <td key={dayIdx} className="p-1">
                                        <div className="bg-orange-500 rounded-xl h-full min-h-[50px] flex items-center justify-center shadow-sm">
                                ...
                                return (
                                    <td key={dayIdx} className="p-1">
                                        <div className="bg-white border border-slate-100 rounded-xl p-2 h-full min-h-[50px] flex flex-col items-center justify-center text-center shadow-sm hover:border-turquoise/50 transition-colors">
    ...
    const renderCharTable = (week: any) => (
        <div className="overflow-x-auto pb-4">
            <table className="w-full min-w-[800px] border-separate border-spacing-1">
                ...
                <tbody>
                    <tr>
                        <td className="p-3 rounded-xl border border-purple-200 bg-purple-50 shadow-sm align-top">
                            <div className="flex items-center gap-3">
                        ...
                        {week.days?.map((day: any, i: number) => {
                            ...
                            return (
                                <td key={i} className="p-1 align-top h-full">
    ...
    const renderMarcheTable = (week: any) => (
        <div className="overflow-x-auto pb-4">
            <table className="w-full min-w-[800px] border-separate border-spacing-1">
                ...
                <tbody>
                    <tr>
                        <td className="p-3 rounded-xl border border-turquoise/20 bg-turquoise/5 shadow-sm align-top">
                            <div className="flex items-center gap-3">
    ...
    return (
        <div className="bg-white rounded-4xl border border-slate-200 shadow-xl overflow-hidden font-sans flex flex-col h-full">
            ...
            <div className="bg-abysse p-6 pb-0 flex flex-col gap-6 shrink-0">
                ...
                {/* TABS SWITCHER */}
                <div className="flex p-1 bg-slate-800/50 rounded-2xl border border-white/5 self-center md:self-start">
                    <button
                        onClick={() => handleTabChange('voile')}
                        className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'voile' ? 'bg-turquoise text-abysse shadow-lg scale-105' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                    >
                        <Sailboat size={16} /> Stages Voile
                    </button>
                    <button
                        onClick={() => handleTabChange('char')}
                        className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'char' ? 'bg-purple-500 text-white shadow-lg scale-105' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                    >
                        <Wind size={16} /> Char à Voile
                    </button>
                    <button
                        onClick={() => handleTabChange('marche')}
                        className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'marche' ? 'bg-turquoise text-abysse shadow-lg scale-105' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                    >
                        <Waves size={16} /> Marche Aquatique
                    </button>
                </div>
