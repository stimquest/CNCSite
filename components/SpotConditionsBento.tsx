...
export const SpotConditionsBento: React.FC<SpotConditionsBentoProps> = ({ mode = 'full' }) => {
    ...
    const showConditions = mode === 'full' || mode === 'conditions';
    const showSocial = mode === 'full' || mode === 'social';

    return (
        <section className="max-w-[1600px] mx-auto px-6 pt-14 pb-6 relative z-10">
            <div className={`grid grid-cols-1 gap-4 items-start ${showConditions && showSocial ? 'lg:grid-cols-12' : showConditions ? 'lg:grid-cols-5' : 'lg:grid-cols-7'}`}>
                ...
                {showSocial && (
                    <div className="lg:col-span-3 p-0 relative group/live flex flex-col transition-all h-full">
                        <div className="relative flex-1 group rounded-4xl overflow-hidden shadow-2xl min-h-[220px] aspect-video lg:aspect-auto">
                            <iframe
